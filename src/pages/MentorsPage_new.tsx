...
import { Search, Filter, Star, Users, Clock, ArrowRight, Loader2, Plus, BarChart3, MessageCircle, X } from 'lucide-react';
import { useMentors } from '../hooks/useMentors';
import { useAuth } from '../contexts/AuthContext';
import MentorChat from '../components/MentorChat';
import { getMentorsWithFallback } from '../lib/sampleMentors';
...
const MentorsPage: React.FC = () => {
  ...
  const { mentors: dbMentors, loading } = useMentors();
  const { user } = useAuth();
  
  // Use sample mentors as fallback if database is empty
  const mentors = getMentorsWithFallback(dbMentors);
  ...
  const filteredMentors = mentors.filter(mentor => {
    ...
    const matchesCategory = selectedCategory === 'all' || 
                           (mentor.expertise && mentor.expertise.some((skill: string) => 
                ...
                <div key={mentor.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-gray-200 transform hover:-translate-y-2 overflow-hidden">
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {mentor.expertise && mentor.expertise.slice(0, 3).map((tag: string, index: number) => (
                        <span
                          key={index}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex space-x-2">
                        <button
                          ...
                          className="group flex items-center space-x-2 px-3 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-all duration-300 font-medium text-sm"
                        >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl h-[600px] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <div className="flex items-center space-x-3">
                <img
                  src={selectedMentorForChat.avatar_url || `https://images.pexels.com/photos/415829/pexels-photo-415829.jpeg?auto=compress&cs=tinysrgb&w=50&h=50&fit=crop`}
                  alt={selectedMentorForChat.name}
                  className="w-10 h-10 rounded-full object-cover"
                />
                <div>
                  <h3 className="font-semibold text-gray-900">{selectedMentorForChat.name} AI</h3>
                  <p className="text-sm text-gray-600">Free trial - No subscription required</p>
                </div>
              </div>
              <button
                onClick={() => setSelectedMentorForChat(null)}
